...
bool Checker::queue_item(const QueueItem & item, CheckedStructure cs)
{
    ...
    if (cs.identity->type() == IDTYPE_CLASS)
    {
        ...
        {
            auto actual_identity = virtual_identity_base::get(reinterpret_cast<virtual_ptr>(const_cast<void *>(item.ptr)));
            if (static_cast<const virtual_identity_base *>(cs.identity)->is_subclass(actual_identity))
            {
    ...
    if (sizes && !cs.inside_structure)
    {
        if (auto allocated_size = get_allocated_size(base))
        {
            ...
            if (cs.identity->type() == IDTYPE_CLASS && get_vtable_name(base, cs, true))
            {
                ...
                auto virtual_type = virtual_identity_base::get(static_cast<virtual_ptr>(const_cast<void *>(base.ptr)));
...
{
    auto identity = static_cast<const container_identity_base *>(cs.identity);
...
void Checker::dispatch_bitfield(const QueueItem & item, const CheckedStructure & cs)
{
    ...
    auto bitfield_type = static_cast<const bitfield_identity_base *>(cs.identity);
...
void Checker::dispatch_enum(const QueueItem & item, const CheckedStructure & cs)
{
    ...
    auto enum_type = static_cast<const enum_identity_base *>(cs.identity);
...
{
    auto identity = static_cast<const struct_identity_base *>(cs.identity);
...
void Checker::dispatch_field(const QueueItem & item, const CheckedStructure & cs, const struct_identity_base *identity, const struct_field_info *field)
{
...
void Checker::dispatch_class(const QueueItem & item, const CheckedStructure & cs)
{
    ...
    auto base_identity = static_cast<const virtual_identity_base *>(cs.identity);
    auto vptr = static_cast<virtual_ptr>(const_cast<void *>(item.ptr));
    auto identity = virtual_identity_base::get(vptr);
    if (!identity)
    {
...
void Checker::dispatch_buffer(const QueueItem & item, const CheckedStructure & cs)
{
    ...
    dispatch_item(item, CheckedStructure(item_identity, identity->byte_size() / item_identity->byte_size(), static_cast<const enum_identity_base *>(identity->getIndexEnumType()), true));
}
...
void Checker::dispatch_tagged_union(const QueueItem & item, const QueueItem & tag_item, const CheckedStructure & cs, const CheckedStructure & tag_cs, const char *attr_name)
{
    ...
    auto union_type = static_cast<const union_identity_base *>(cs.identity);
    ...
    auto tag_identity = static_cast<const enum_identity_base *>(tag_cs.identity);
...
{
    auto union_container_identity = static_cast<const container_identity_base *>(cs.identity);
    ...
    auto tag_container_identity = static_cast<const container_identity_base *>(tag_cs.identity);
...
void Checker::check_stl_vector(const QueueItem & item, const type_identity *item_identity, const type_identity *eid)
{
...
void Checker::check_stl_map(const QueueItem & item, const container_identity_base *identity)
{
#ifndef WIN32
...
void Checker::check_stl_unordered_map(const QueueItem & item, const container_identity_base *identity)
{
#ifndef WIN32
    const static CheckedStructure cs(identity, 0, nullptr, true);
