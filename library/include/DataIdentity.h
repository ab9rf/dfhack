...
namespace df {
  ...
  template <typename T> struct identity_traits;
}
...
namespace DFHack
{
    ...
    protected:
        primitive_identity_base(size_t size, const std::type_info& id) : type_identity(size, id) {};
    ...
    public:
        primitive_identity() : primitive_identity_base(sizeof(T), typeid(T)) {};
    };

    class DFHACK_EXPORT opaque_identity_base : public constructed_identity_base {
        const std::string name;

    protected:
        opaque_identity_base(size_t size, const std::type_info& id, TAllocateFn alloc, const std::string& name)
            : constructed_identity_base(size, id, alloc), name(name) {};
    ...
    template<typename T>
    class opaque_identity : public opaque_identity_base {
    public:
        // all this extra effort because c++ won't let sizeof(void) == 0...
        opaque_identity(const std::string& name) requires (!std::is_void_v<T>) : opaque_identity_base(sizeof(T), typeid(T), &df::allocator_fn<T>, name) {};
        opaque_identity(const std::string& name) requires (std::is_void_v<T>) : opaque_identity_base(0, typeid(T), nullptr, name) {};
    };

    class DFHACK_EXPORT shared_ptr_identity_base : public constructed_identity_base {
        const type_identity* underlying;
    protected:
        shared_ptr_identity_base(size_t size, const std::type_info& id, TAllocateFn alloc, const type_identity* underlying)
            : constructed_identity_base(size, id, alloc), underlying(underlying) {};

    public:
        std::string getFullName() const {
            return std::string("shared_ptr<") + underlying->getFullName() + ">";
        }
    ...
    template<typename T>
    class shared_ptr_identity : public shared_ptr_identity_base {
        using underlying_type = typename T::element_type;
    public:
        shared_ptr_identity() : shared_ptr_identity_base(sizeof(T), typeid(T), &df::allocator_fn<T>, df::identity_traits<underlying_type>::get()) {};
    };
    ...
    class DFHACK_EXPORT pointer_identity_base : public primitive_identity_base {
        ...
        friend void LuaWrapper::push_adhoc_pointer(lua_State* state, void* ptr, const type_identity* target);

        // this is public solely for check-structures-sanity
    public:
        pointer_identity_base(const type_identity* target = NULL, const std::type_info& id = typeid(void*))
            : primitive_identity_base(sizeof(void*), id), target(target) {};
    ...
    private:
        static constexpr std::string makeName()
        {
            const type_identity* u_type = df::identity_traits<underlying_type>::get();
            return u_type->getFullName() + "*";
        };
    public:
        pointer_identity() : pointer_identity_base(df::identity_traits<underlying_type>::get(), typeid(T)) {};
        std::string getFullName() const { return makeName(); }
    };

    class DFHACK_EXPORT container_identity_base : public constructed_identity_base {
        const type_identity* item;
        const enum_identity_base* ienum;

    protected:
        container_identity_base(size_t size, const std::type_info& id, TAllocateFn alloc, const type_identity* item, const enum_identity_base* ienum = NULL)
            : constructed_identity_base(size, id, alloc), item(item), ienum(ienum) {};

    public:
        virtual identity_type type() const { return IDTYPE_CONTAINER; }

        std::string getFullName() const { return getFullNameImpl(item); }
        ...
        virtual std::string getFullNameImpl(const type_identity* item) const;
    ...
    template<typename T>
    class container_identity : public container_identity_base {
    public:
        container_identity(enum_identity_base* ienum = NULL) : container_identity_base(sizeof(T), typeid(T), &df::allocator_fn<T>, df::identity_traits<T>::get(), ienum) {};
    };

    class DFHACK_EXPORT ptr_container_identity_base : public container_identity_base {
    protected:
        ptr_container_identity_base(size_t size, const std::type_info& id, TAllocateFn alloc,
            const type_identity* item, const enum_identity_base* ienum = NULL)
            : container_identity_base(size, id, alloc, item, ienum) {};
    public:
        virtual identity_type type() const { return IDTYPE_PTR_CONTAINER; }

        std::string getFullNameImpl(const type_identity* item) const;
    ...
    template<typename T>
    class ptr_container_identity : public ptr_container_identity_base {
    public:
        ptr_container_identity(enum_identity_base* ienum = NULL) : ptr_container_identity_base(sizeof(T), typeid(T), &df::allocator_fn<T>, df::identity_traits<T>::get(), ienum) {};
    };

    class DFHACK_EXPORT bit_container_identity_base : public container_identity_base {
    protected:
        bit_container_identity_base(size_t size, const std::type_info& id, TAllocateFn alloc, const enum_identity_base* ienum = NULL)
            : container_identity_base(size, id, alloc, NULL, ienum) {};
    ...
    template<typename T>
    class bit_container_identity : public bit_container_identity_base {
    public:
        bit_container_identity(enum_identity_base* ienum = NULL) : bit_container_identity_base(sizeof(T), typeid(T), &df::allocator_fn<T>, ienum) {};
    };
...
namespace df
{
    ...
    using DFHack::opaque_identity_base;
    using DFHack::pointer_identity_base;
    using DFHack::container_identity_base;
    using DFHack::ptr_container_identity_base;
    using DFHack::bit_container_identity_base;
    using DFHack::shared_ptr_identity_base;
    using DFHack::primitive_identity;
    using DFHack::opaque_identity;
    using DFHack::pointer_identity;
    using DFHack::shared_ptr_identity;

    class DFHACK_EXPORT number_identity_base : public primitive_identity_base {
        const char *name;

    public:
        number_identity_base(size_t size, const std::type_info& id, const char *name)
            : primitive_identity_base(size, id), name(name) {};
    ...
    public:
        integer_identity_base(size_t size, const std::type_info& id, const char *name)
            : number_identity_base(size, id, name) {}
    ...
    public:
        float_identity_base(size_t size, const std::type_info& id, const char *name)
            : number_identity_base(size, id, name) {}
    ...
    public:
        integer_identity(const char *name) : integer_identity_base(sizeof(T), typeid(T), name) {}
    ...
    public:
        float_identity(const char *name) : float_identity_base(sizeof(T), typeid(T), name) {}
    ...
    class DFHACK_EXPORT bool_identity : public primitive_identity<bool> {
    public:
        bool_identity() : primitive_identity<bool>() {};
    ...
    class DFHACK_EXPORT ptr_string_identity : public primitive_identity<char*> {
    public:
        ptr_string_identity() : primitive_identity<char*>() {};
    ...
    class DFHACK_EXPORT stl_string_identity : public DFHack::constructed_identity<std::string> {
    public:
        stl_string_identity() : constructed_identity<std::string>() {};
    ...
    class DFHACK_EXPORT stl_ptr_vector_identity : public ptr_container_identity_base {
    public:
        ...
        stl_ptr_vector_identity(const std::type_info& id = typeid(std::vector<void*>), const type_identity* item = NULL, const enum_identity_base* ienum = NULL)
            : ptr_container_identity_base(sizeof(container), typeid(id), allocator_fn<container>, item, ienum)
        {};

        std::string getFullNameImpl(const type_identity *item) const {
            return "vector" + ptr_container_identity_base::getFullNameImpl(item);
        }
...
// Due to export issues, this stuff can only work in the main dll
    ...
    class buffer_container_identity : public container_identity_base {
        const int size;

    public:
        buffer_container_identity()
            : container_identity_base(0, typeid(char*), NULL, NULL, NULL), size(0) // not sure char is appropriate here?
        {}
        // size is used a bit oddly in this class
        buffer_container_identity(int size, const std::type_info& id, const type_identity *item, const enum_identity_base *ienum = NULL)
            : container_identity_base(size, id, NULL, item, ienum), size(size)
        {}
    ...
    template<class T>
    class stl_container_identity : public container_identity_base {
        const char *name;

    public:
        stl_container_identity(const char *name, const type_identity *item, const enum_identity_base *ienum = NULL)
            : container_identity_base(sizeof(T), typeid(T), &allocator_fn<T>, item, ienum), name(name)
        {}

        std::string getFullNameImpl(const type_identity *item) {
            return name + container_identity_base::getFullNameImpl(item);
        }
...
#ifdef BUILD_DFHACK_LIB
    template<class T>
    class ro_stl_container_identity : public container_identity_base {
    protected:
        const char *name;

    public:
        ro_stl_container_identity(const char *name, const type_identity *item, const enum_identity_base *ienum = NULL)
            : container_identity_base(sizeof(T), typeid(T), & allocator_fn<T>, item, ienum), name(name)
        {}
    ...
    public:
        ...
        virtual std::string getFullNameImpl(const type_identity*) const override {
    ...
    class bit_array_identity : public bit_container_identity_base {
    public:
        /*
        ...
        bit_array_identity(const enum_identity_base *ienum = NULL)
            : bit_container_identity_base(sizeof(container), typeid(container), & allocator_fn<container>, ienum)
        {}
    ...
    class DFHACK_EXPORT stl_bit_vector_identity : public bit_container_identity_base {
    public:
        typedef std::vector<bool> container;

        stl_bit_vector_identity(const enum_identity_base *ienum = NULL)
            : bit_container_identity_base(sizeof(container), typeid(container), & allocator_fn<container>, ienum)
        {}

        std::string getFullNameImpl(const type_identity *item) const {
            return "vector" + bit_container_identity_base::getFullNameImpl(item);
        }
...
#ifdef BUILD_DFHACK_LIB
    template<class T>
    class enum_list_attr_identity : public container_identity_base {
    public:
        typedef enum_list_attr<T> container;

        enum_list_attr_identity(const type_identity *item)
            : container_identity_base(sizeof(container), typeid(container), NULL, item, NULL)
        {}

        std::string getFullNameImpl(const type_identity *item) const {
            return "enum_list_attr" + container_identity_base::getFullNameImpl(item);
        }
...
#define OPAQUE_IDENTITY_TRAITS(...) \
    template<> struct DFHACK_EXPORT identity_traits<__VA_ARGS__ > { \
        static const opaque_identity<__VA_ARGS__ > identity; \
        static const opaque_identity_base *get() { return &identity; } \
    };
...
#ifdef BUILD_DFHACK_LIB
    template<typename T>
    struct identity_traits<std::shared_ptr<T> > {
        static const shared_ptr_identity_base* get() {
            using ptr_type = std::shared_ptr<T>;
            static const shared_ptr_identity<ptr_type> identity{};
            return &identity;
        }
    };
#endif
    ...
    template<> struct DFHACK_EXPORT identity_traits<void*> {
        ...
        static const pointer_identity_base* get() { return &identity; }
    };
    ...
    template<> struct DFHACK_EXPORT identity_traits<void> {
        static const opaque_identity<void> identity;
        static const opaque_identity<void>* get() { return &identity; }
    };
...
#undef OPAQUE_IDENTITY_TRAITS
    ...
    template<class Enum, class FT> struct identity_traits<enum_field<Enum,FT> > {
        static const enum_identity_base *get();
    };
#endif
    ...
    template<class T, int sz> struct identity_traits<T [sz]> {
        static const container_identity_base *get();
    };
    ...
    template<> struct identity_traits<BitArray<int> > {
        ...
        static const bit_container_identity_base *get() { return &identity; }
    };

    template<class T> struct identity_traits<BitArray<T> > {
        static const bit_container_identity_base *get();
    };
    ...
    inline const enum_identity_base *identity_traits<enum_field<Enum,FT> >::get() {
        static const enum_identity_base identity(identity_traits<Enum>::get(), typeid(Enum), identity_traits<FT>::get());
        return &identity;
    }
#endif
    ...
    inline const container_identity_base *identity_traits<T [sz]>::get() {
        static const buffer_container_identity identity(sz, typeid(T [sz]), identity_traits<T>::get());
        return &identity;
    }

    template<class T>
    inline const container_identity_base *identity_traits<std::vector<T> >::get() {
    ...
    inline const stl_ptr_vector_identity *identity_traits<std::vector<T*> >::get() {
        static const stl_ptr_vector_identity identity(typeid(std::vector<T*>), identity_traits<T>::get());
        return &identity;
    }
    ...
    inline const container_identity_base* identity_traits<std::vector<int32_t> >::get() {
    ...
    inline const container_identity_base* identity_traits<std::vector<int16_t> >::get() {
        return &stl_vector_int16_t_identity;
    }

#ifdef BUILD_DFHACK_LIB
    template<class T>
    inline const container_identity_base *identity_traits<std::deque<T> >::get() {
    ...
    template<class T>
    inline const container_identity_base *identity_traits<std::set<T> >::get() {
    ...
    inline const container_identity_base *identity_traits<std::map<KT, T>>::get() {
    ...
    template<class KT, class T>
    inline const container_identity_base *identity_traits<std::unordered_map<KT, T>>::get() {
    ...
    template<class T>
    inline const bit_container_identity_base *identity_traits<BitArray<T> >::get() {
        static const bit_array_identity identity(identity_traits<T>::get());
        return &identity;
    }

    template<class T>
    inline const container_identity_base *identity_traits<DfArray<T> >::get() {
    ...
    template<class T>
    inline const container_identity_base *identity_traits<enum_list_attr<T> >::get() {
