...
void LuaWrapper::push_object_internal(lua_State *state, const type_identity *type, void *ptr, bool in_method)
{
    ...
    {
        virtual_identity_base *class_vid = virtual_identity_base::get(virtual_ptr(ptr));
        if (class_vid)
            type = class_vid;
    }
...
static void fetch_container_details(lua_State *state, int meta, const type_identity * *pitem, int *pcount)
{
    ...
    *pitem = (const type_identity*)lua_touserdata(state, -1);
    lua_pop(state, 1);
                                    ...
                                    const type_identity *type2, int meta2, bool exact_equal)
{
    ...
    case IDTYPE_CLASS:
    {
        auto b1 = (struct_identity_base*)type1;
        auto b2 = (struct_identity_base*)type2;
...
static int meta_ptr_tostring(lua_State *state)
{
    ...
    uint64_t length = 0;
    auto *cid = dynamic_cast<const df::container_identity_base*>(get_object_identity(state, 1, "__tostring()", true, true));
...
static int meta_enum_attr_index(lua_State *state)
{
    ...
    auto id = (enum_identity_base*)lua_touserdata(state, lua_upvalueindex(2));
...
{
    const auto *complex = (enum_identity_base::ComplexData*)lua_touserdata(L, lua_upvalueindex(2));
...
static int complex_enum_inext(lua_State *L)
{
    ...
    int64_t i = (is_first)
        ? ((enum_identity_base::ComplexData*)lua_touserdata(L, lua_upvalueindex(2)))->index_value_map[0]
        : luaL_checkint(L, 2);
...
static void RenderTypeChildren(lua_State *state, const std::vector<const compound_identity_base*> &children);
...
static void FillEnumKeys(lua_State *state, int ix_meta, int ftable, enum_identity_base *eid)
{
...
static void FillBitfieldKeys(lua_State *state, int ix_meta, int ftable, bitfield_identity_base *eid)
{
    // Create a new table attached to ftable as __index
    lua_newtable(state);
    int base = lua_gettop(state);
...
static void RenderType(lua_State *state, const compound_identity_base *node)
{
    assert(node->getName());
    ...
    SaveInTable(state, const_cast<compound_identity_base*>(node), &DFHACK_TYPEID_TABLE_TOKEN);
    ...
    lua_pushlightuserdata(state, const_cast<compound_identity_base*>(node));
    ...
    case IDTYPE_UNION: // TODO: change this to union-type? what relies on this?
        ...
        IndexStatics(state, ix_meta, ftable, (struct_identity_base*)node);
        break;
    ...
    case IDTYPE_ENUM:
        ...
        lua_setfield(state, ftable, "_kind");
        FillEnumKeys(state, ix_meta, ftable, (enum_identity_base*)node);
        break;

    case IDTYPE_BITFIELD:
        lua_pushstring(state, "bitfield-type");
        lua_setfield(state, ftable, "_kind");
        FillBitfieldKeys(state, ix_meta, ftable, (bitfield_identity_base*)node);
        break;
